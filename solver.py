...
class Solver(object):
    ...
    def get_lpips_loss(self, batch1, batch2):
        return self.lpips_model(batch1, batch2).mean()
    ...
    def get_loss(self, batch1, batch2, dis=0):
        ...
        lpips_loss = self.get_lpips_loss(s1, s2)
        psnr = ((s1 - s2) ** 2).mean(1).mean(1).mean(1)
        psnr = 10 * (torch.log10(1.0 / psnr))
        loss = - ssim - si - psnr / 40.0 + 3 + 2 * perceptual_loss + lpips_loss
        # score = psnr
        return loss, ssim, perceptual_loss, psnr, lpips_loss
    ...
    def train(self):
        ...
        fixed_mix, fixed_gro, fixed_alpha = fixed_mix.to(self.device), fixed_gro.to(self.device), fixed_alpha.to(
            self.device).float().unsqueeze(1)
        ...
        for e in range(start_iters, self.config.num_epochs):
            for i, (data_mix, data_gro, data_alpha_g) in enumerate(self.data_loader_train):
                data_mix, data_gro, data_alpha = data_mix.to(self.device), data_gro.to(self.device), data_alpha_g.to(
                    self.device).float().unsqueeze(1)
                loss = {}
                onenumber = torch.ones([data_mix.size(0), 1], dtype=torch.float32).to(self.device)
                output_real_D = self.D(data_mix, data_alpha)
                d_loss_real, d_ssim, _, _, d_lpips = self.get_loss(output_real_D, data_gro, self.dt)
                ...
                if (i + 1) % 5 == 0:
                    ...
                    loss['g_lpips'] = g_lpips.mean().item()
            ...
            with torch.no_grad():
                ...
                file_extension = '.png' if (e + 1) % 2 == 0 else '.jpg' 
                sample_path = os.path.join(self.config.sample_dir, f'{e + 1}-images{file_extension}')
                ...
                print('SSIM mix&gro:{}, \t SSIM:{}'.format(
                    self.ssim_loss(self.denorm(fixed_mix), self.denorm(fixed_gro)).mean(),
                    self.ssim_loss(fixed_output, self.denorm(fixed_gro)).mean()))
    ...
    def compute_loss1(self, data_loadert):
        ...
        with torch.no_grad():
            for i, (mix, gro, data_alpha) in enumerate(data_loadert):
                mix, gro, data_alpha = mix.to(self.device), gro.to(self.device), data_alpha.to(
                    self.device).float().unsqueeze(1)
    ...
    def val(self, i):
        ...
        f1.write(str(i) + '\t' + str(loss1) + '\t' + str(loss2) + '\t' + str(loss3) + '\t' + str(score1) + '\t' + str(
            score2) + '\t' + str(score3) + '\n')
        ...
        f2.write(str(i) + '\t' + str(loss1) + '\t' + str(loss2) + '\t' + str(loss3) + '\t' + str(score1) + '\t' + str(
            score2) + '\t' + str(score3) + '\n')
